...
- Puzzle cards (`.puzzle-card`) redesigned as postcards/stamps:
  ...
  - The image frame (`.card-image-wrap`) now uses the same diagonal red/white stripe pattern as the home screen background as its padding/frame color, so each card echoes the home screen treatment.
